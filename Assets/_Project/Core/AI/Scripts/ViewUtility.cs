...
namespace _Project.Core.AI.Scripts
{
    ...
    {
        public static bool IsVisibleUnit<T>(T unit, Transform from, float angle, float distance, LayerMask mask) where T : IVisible
        {
            var result = false;
            ...
            if (unit != null)
            {
                ...
                {
                    if (IsVisibleObject(from, visiblePoint.position, unit.GetGameObject(), angle, distance, mask))
                    {
                        result = true;
                        break;
                    }
